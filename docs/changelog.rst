...
------------------
    ...
    * **Improvement**  -  ``RiverApp._register_hook_inlines`` deduplicates the injected admin ``inlines``/``readonly_fields`` with an order-preserving ``dict.fromkeys(...)`` instead of ``set(...)``, so the resulting admin column/inline order no longer depends on Python's per-process hash randomization.
    * **Security**      -  Sandbox hardening: ``RIVER_SANDBOX_DB_FUNCTIONS``'s restricted ``_getattr_`` is now ``RestrictedPython.Guards.safer_getattr`` instead of the plain builtin ``getattr``. The source-level dunder guard alone didn't stop an *indirect* dunder lookup assembled at runtime — e.g. ``"{0.__class__}".format(context)`` reaches ``__class__`` through ``str.format``'s own C-level attribute lookup, never touching the AST-level check. ``safer_getattr`` rejects any attribute name starting with ``_`` at call time regardless of how it was constructed, closing that gap.
    * **Feature**       -  New system check ``xii_django_river.W005`` (``check_hooks_with_unapproved_functions``): flags any ``Hook`` still pointing at a ``Function`` whose ``is_approved`` was reset by a later body edit — ``Hook.save()`` only validated this at the moment the hook itself was saved, so a hook could silently end up pointing at now-unapproved code with nothing surfacing that fact until it actually fired.
    * **Feature**       -  New system check ``xii_django_river.W006`` (``check_timeout_with_offthread_executor``): flags ``RIVER_FUNCTION_TIMEOUT_SECONDS`` set together with ``RIVER_HOOK_EXECUTOR``, since the timeout is enforced with ``signal.alarm`` (main thread only) and silently stops applying once hook execution moves off-thread/off-process.
    * **Feature**       -  ``create_function(callback)`` now writes an explicit ``FunctionRevision`` with ``action=ACTION_AUTO_APPROVED`` instead of only flipping ``is_approved``/``approved_at`` directly, so the revision history makes it unambiguous that the approval came from this helper rather than a human reviewer or a `Function` that was never put through any approval flow at all (migration ``0004``).
    * **Infra**         -  ``xii`` converted to a `PEP 420 <https://peps.python.org/pep-0420/>`_ namespace package (``xii/__init__.py`` removed), so other ``xii.*``-prefixed distributions can coexist without conflict. CI and ``tox.ini`` now run tests with ``--top-level-directory=.``, required because unittest's dotted-label discovery otherwise auto-detects the wrong top-level directory under a namespace package and double-imports every test module under a colliding name.
