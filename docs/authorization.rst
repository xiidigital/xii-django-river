Authorization
=============
``xii-django-river`` provides system users an ability to configure the authorizations at three level. Those are permissions, user group or a specific user at any step. If the
...
""""""""""""""""""""""""""""""
Multiple permission can be specified on the `transition approval metadata` admin page and ``xii-django-river`` will allow only the users who have the given permission.
...
configurable on the admin page provided by `xii-django-river`

User Group Based Authorization
""""""""""""""""""""""""""""""
Multiple user group can be specified on the `transition approval metadata` admin page and ``xii-django-river`` will allow only the users who are in the given user groups.
...
authorized for the user.This can be configurable on the admin page provided by `xii-django-river`
...
methods, ``xii-django-river`` doesn't provide an admin interface for that. But this can be handled within the repositories that is using `xii-django-river`. The way how to do
