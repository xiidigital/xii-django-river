...
from xii.django_river.models.function import FunctionRevision
...
class FunctionAdmin(admin.ModelAdmin):
    ...
    def has_approve_permission(self, request):
        return request.user.has_perm('xii_django_river.approve_function')

    def has_self_approve_permission(self, request):
        return request.user.has_perm('xii_django_river.self_approve_function')
    ...
    def approve_functions(self, request, queryset):
        if not self.has_approve_permission(request):
            self.message_user(
                request,
                "You don't have the xii_django_river.approve_function permission.",
        ...
        if skipped:
            self.message_user(
                ...
                "xii_django_river.self_approve_function permission — ask a different reviewer to "
